use crate::frontend::bcode::ITag;

use super::{
    bcode::{CTag, Code, CodeBundle, Index},
...
impl Parser {
    ...
    fn parse_equality(&mut self) -> Result<Index, ParseError> {
        ...
        while matches!(self.cur_token(), Token::Equal(_) | Token::NotEqual(_)) {
            ...
            let ins = Code {
                tag: match op {
                    Token::Equal(_) => CTag::Eq,
                    Token::NotEqual(_) => CTag::Neq,
                    _ => unreachable!(),
                },
                indices: vec![left, right],
                src: op.get_source_ref(),
    ...
    fn parse_comparison(&mut self) -> Result<Index, ParseError> {
        ...
        while matches!(
            ...
            let ins = Code {
                tag: match op {
                    Token::Less(_) => CTag::Lt,
                    Token::LessEqual(_) => CTag::LtEq,
                    Token::Greater(_) => CTag::Gt,
                    Token::GreaterEqual(_) => CTag::GtEq,
                    _ => unreachable!(),
                },
                indices: vec![left, right],
                src: op.get_source_ref(),
    ...
    fn parse_term(&mut self) -> Result<Index, ParseError> {
        ...
        while matches!(self.cur_token(), Token::Plus(_) | Token::Minus(_)) {
            ...
            let ins = Code {
                tag: match op {
                    Token::Plus(_) => CTag::Add,
                    Token::Minus(_) => CTag::Sub,
                    _ => unreachable!(),
                },
                indices: vec![left, right],
                src: op.get_source_ref(),
    ...
    fn parse_factor(&mut self) -> Result<Index, ParseError> {
        ...
        while matches!(
            ...
            let ins = Code {
                tag: match op {
                    Token::Star(_) => CTag::Mult,
                    Token::Slash(_) => CTag::Div,
                    Token::Modulo(_) => CTag::Modulo,
                    _ => unreachable!(),
                },
                indices: vec![left, right],
                src: op.get_source_ref(),
        ...
        match cur {
            ...
            Token::I8(src) => {
                ...
                let n_type = TypeSignature {
                    tag: TSTag::Type,
                    src: src.clone(),
                    indices: vec![self.code.add_type(a_type)],
                };
                let n_type_i = self.code.add_type(n_type);
                if !generate_value {
                    return Ok(n_type_i);
