use proto::{
	...
	parser::{ParseContext, parse, record}, ast::AstNode,
};
use std::fs;

#[test]
fn test_parser_record_id_and_members() -> Result<(), ProtoErr> {
	let path = "./samples/test_sources/parser/valid/record_id_and_members.pr";
	let src = fs::read_to_string(path).unwrap();
    
	let mut lexer_ctx = LexContext::new(src.clone());
	let mut parser_ctx = ParseContext { current: lex_next(&mut lexer_ctx).unwrap() };

	if let AstNode::Struct(rec) = record(&mut lexer_ctx, &mut parser_ctx)? {
